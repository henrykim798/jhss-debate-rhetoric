...
(1) Second independent annotation (by Claude, an LLM rater distinct from the GPT-3.5 production
...
# ---- Claude's independent labels for the 80-sentence sample (order matches val_sample.json) ----
...
def confusion(rows,cols,rcats,ccats):
    ...
    return M
# rows = GPT (production), cols = Claude (second rater)
...
P("Second independent rater: Claude (LLM), blind to GPT-3.5 production labels.")
P(f"Sample: {len(gpt)} sentences, stratified across the four production categories.\n")
P("Confusion matrix — rows = GPT-3.5 production label, cols = independent rater (Claude):")
P(CM.to_string()); P("")
...
P(f"N = {len(df)} sentences. Rows = GPT-3.5, cols = rule-based classifier:")
P(CM2.to_string()); P("")
